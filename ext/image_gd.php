...
define('CACHE_DIR_NAME',		'/public/cache/');
define('CACHE_DIR',				CURRENT_DIR . CACHE_DIR_NAME);
define('DOCUMENT_ROOT',			dirname(__FILE__)."/"); // $_SERVER['DOCUMENT_ROOT']
...
if (isset($_GET['cropratio']))
{
	...
	if (count($cropRatio) == 2)
	{
		...
		$cropX  = isset($_GET["cropx"]) ? (int)$_GET["cropx"] : -1;
		$cropY  = isset($_GET["cropy"]) ? (int)$_GET["cropy"] : -1;
		...
		{ // Image is too tall so we will crop the top and bottom
			...
			if ($cropY < 0)
				...
				$offsetY = round(($origHeight - $height) * ($cropY/100));
		}
		...
		{ // Image is too wide so we will crop off the left and right sides
			...
			if ($cropX < 0)
				...
				$offsetX = round(($origWidth - $width) * ($cropX/100));
		}
	}
}
